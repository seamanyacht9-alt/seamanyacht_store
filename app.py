...
if menu == "📊 แดชบอร์ด (ภาพรวม)":
    ...
    total_po_spent = po_log_df[~po_log_df['Status'].isin(['Voided (ยกเลิก)', '❌ ตีกลับ (ขอเงินคืน)'])]['Net_Price'].sum() if not po_log_df.empty else 0
...
# ==========================================
    ...
    with col2:
        ...
        if po_cart_df.empty:
            ...
            display_po_cart = po_cart_df.rename(columns={"Requester": "ผู้ขอซื้อ", "Item_Name": "รายการ", "Qty": "จำนวน", "Unit": "หน่วย", "Price_Per_Unit": "ราคา/หน่วย", "Net_Price": "ราคาสุทธิ", "Shop_Name": "ร้านค้า"}).drop(columns=['id', 'Discount', 'Shipping'])
            ...
            # --- ฟีเจอร์ใหม่: ลบรายการเฉพาะเจาะจงในตะกร้า PO ---
            ...
            with c_b:
                if st.button("💾 บันทึกใบสั่งซื้อ (Save PO)", type="primary", use_container_width=True):
                    ...
                    for idx, row in po_cart_df.iterrows():
                        ...
                        # ถ้ารายการนี้ไม่มีในสต๊อก ให้สร้างใหม่ทันที (ยอด=0)
                        if row['Item_Name'] not in inventory_df['Item_Name'].values:
                            new_code = f"NEW-{tx_id}"
                            supabase.table("inventory_db").insert({
                                "Item_Code": new_code, "Item_Name": row['Item_Name'], "Zone": "รอจัดหมวดหมู่", 
                                "Stock": 0, "Min_Stock": 0, "Unit": row['Unit']
                            }).execute()
    ...
    if po_log_df.empty:
        ...
        if selected_po_to_print:
            ...
            <html><head><style>
                ...
                .info-box {{ width: 100%; margin-bottom: 15px; border-bottom: 2px solid #ddd; padding-bottom: 10px; text-align: center; }}
            ...
            <body>
                ...
                <div class="info-box">
                    ...
                    <strong>วันที่ออกเอกสาร:</strong> {date_str}
                </div>
                <table>
                    <tr>
                        <th style="width: 5%;">ลำดับ</th>
                        <th style="width: 25%;">รายการสินค้า</th>
                        <th style="width: 15%;">ผู้ขอซื้อ</th>
            ...
            """
            # แก้ไขบั๊กตัวเลขลำดับ PDF ตรงนี้ (ใช้ตัวนับแทนการตัด String)
            counter = 1
            ...
            html_invoice += f"""
                    <tr class="total-row">
                        <td colspan="7" style='text-align: right;'>ยอดรวมสุทธิ (Grand Total):</td>
            ...
            """
            components.html(html_invoice, height=450, scrolling=True)

        st.markdown("---")
        
        # ---------------------------------------------
        # โซน "รับของเข้าคลัง" และ "ตารางประวัติ"
        # ---------------------------------------------
        ...
        with col2:
            ...
            st.dataframe(display_po_df.iloc[start_po_idx:end_po_idx][['รหัสรายการ', 'เลขที่ PO', 'ผู้ขอซื้อ', 'รายการ', 'จำนวน', 'ราคาสุทธิ', 'ร้านค้า', 'สถานะ']], use_container_width=True, hide_index=True)
        ...
        with tab1:
            st.caption("ใช้เมื่อเผลอกด 'รับของเข้าคลัง' ผิดรายการ (ระบบจะหักของออกจากสต๊อก และเปลี่ยนสถานะกลับเป็น 'รอรับของ')")
        ...
        with tab2:
            st.caption("ใช้เมื่อร้านส่งของมาไม่ตรงสเปก ชำรุด และต้องส่งคืนเพื่อขอเงินคืน")
        ...
        with tab3:
            st.caption("ใช้เมื่อต้องการยกเลิกบิลสั่งซื้อ")
        ...
        with tab4:
            st.caption("ลบข้อมูลออกจากฐานข้อมูลแบบถาวร")
...
# ==========================================
    ...
    if po_log_df.empty:
        ...
        # ดึงเฉพาะบิลที่ไม่ถูกยกเลิกมาคำนวณ
        ...
        if valid_pos.empty:
            ...
            # --- ฟีเจอร์ใหม่: ยกเลิกประวัติการจ่ายเงิน (Void Payment) ---
            st.markdown("---")
...
# ==========================================
    ...
    if transaction_df.empty:
        ...
        display_history_df = display_df.iloc[::-1].rename(columns={"TxID": "รหัสบิล", "Timestamp": "เวลา", "Action": "ประเภท", "Item_Name": "ชื่อวัสดุ", "Qty": "จำนวน", "Worker": "ผู้เบิก/ผู้รับ", "Status": "สถานะ", "Boat_Name": "ชื่อเรือ"})
