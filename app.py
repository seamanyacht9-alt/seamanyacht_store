...
# ==========================================
        ...
        with st.form("add_po_form", clear_on_submit=True):
            ...
            if st.form_submit_button("➕ นำลงใบสั่งซื้อ"):
                if not requester or not selected_item or not shop_name:
                    ...
                    # คำนวณ Net Price โดยรวม VAT เข้าไปด้วย
    ...
    with col2:
        ...
        if po_cart_df.empty:
            ...
            # ดึง VAT มาแสดงในตารางตะกร้าด้วย
            po_cart_df['VAT'] = po_cart_df.get('VAT', 0)
            display_po_cart = po_cart_df.rename(columns={"Requester": "ผู้ขอซื้อ", "Item_Name": "รายการ", "Qty": "จำนวน", "Unit": "หน่วย", "Price_Per_Unit": "ราคา/หน่วย", "Net_Price": "ราคาสุทธิ", "Shop_Name": "ร้านค้า"}).drop(columns=['id', 'Discount', 'Shipping', 'VAT'])
            ...
            # --- ฟีเจอร์: ลบรายการเฉพาะเจาะจงในตะกร้า PO ---
            ...
            with c_b:
                if st.button("💾 บันทึกใบสั่งซื้อ (Save PO)", type="primary", use_container_width=True):
                    ...
                    for idx, row in po_cart_df.iterrows():
                        ...
                        # บันทึก VAT ลงในประวัติ PO_LOG ด้วย
