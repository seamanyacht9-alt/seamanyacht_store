...
# ==========================================
    ...
    if po_log_df.empty:
        ...
        if selected_po_to_print:
            ...
            # --- เริ่มก๊อปปี้ทับตั้งแต่บรรทัดนี้ลงไป ---
            html_invoice = f"""
            <!DOCTYPE html>
            <html lang="th">
            <head>
                ...
                <style>
                    body {{ font-family: 'Sarabun', sans-serif; color: #333; padding: 20px; background-color: white; }}
                    #invoice-content {{ padding: 20px; background-color: white; }}
                    h2 {{ text-align: center; color: #1a365d; margin-bottom: 5px; font-size: 24px; }}
                    .info-box {{ width: 100%; margin-bottom: 15px; border-bottom: 2px solid #ddd; padding-bottom: 10px; text-align: center; font-size: 15px; }}
                    ...
                    .btn-print, .btn-download {{ padding: 12px 24px; color: white; text-decoration: none; border-radius: 5px; cursor: pointer; font-weight: bold; border: none; font-family: 'Sarabun', sans-serif; font-size: 15px; transition: 0.3s; }}
            ...
            html_invoice += f"""
                ...
                <script>
                    // ตั้งค่ามาตรฐานให้ PDF ไม่แหว่ง
                    ...
                    function downloadPDF() {{
                        window.scrollTo(0, 0); // แก้บั๊กเลื่อนจอแล้วขอบแหว่ง หรือหน้าขาว
                        var element = document.getElementById('invoice-content');
                        html2pdf().set(getPDFOptions()).from(element).save();
                    }}

                    // ฟังก์ชันเปิดแท็บใหม่เพื่อ Print 
                    function printPDF() {{
                        window.scrollTo(0, 0); // แก้บั๊กเลื่อนจอ
                        var element = document.getElementById('invoice-content');
                        // สร้าง PDF แล้วเปิดเป็นไฟล์ในแท็บใหม่ จะได้ปริ้นแบบ 100% เต็มหน้า
